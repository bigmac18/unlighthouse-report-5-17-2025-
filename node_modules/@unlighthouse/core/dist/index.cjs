...
const revisions_js = require('puppeteer-core/lib/cjs/puppeteer/revisions.js');
...
const medianRun_js = require('lighthouse/core/lib/median-run.js');
const cheerio = require('cheerio');

var _documentCurrentScript = typeof document !== 'undefined' ? document.currentScript : null;
function _interopDefaultCompat (e) { return e && typeof e === 'object' && 'default' in e ? e.default : e; }

const dns__default = /*#__PURE__*/_interopDefaultCompat(dns);
...
async function generateClient(options = {}, unlighthouse) {
  ...
  let prefix = ufo.withTrailingSlash(ufo.withLeadingSlash(resolvedConfig.routerPrefix));
  if (prefix === "/") {
    prefix = "";
  }
  const clientPathFolder = path.dirname(runtimeSettings.resolvedClientPath);
  ...
  await fs__default.writeFile(path.resolve(runtimeSettings.generatedClientPath, "index.html"), indexHTML, "utf-8");
  const staticData = {
    reports: [],
    scanMeta: createScanMeta(),
    // need to be selective about what options we put here to avoid exposing anything sensitive
    options: lodashEs.pick({
      ...runtimeSettings,
      ...resolvedConfig
    }, [
      "client",
  ...
  await fs__default.writeFile(
    path.join(runtimeSettings.generatedClientPath, "assets", "payload.js"),
    `window.__unlighthouse_payload = ${JSON.stringify(staticData)}`,
    { encoding: "utf-8" }
  );
  const globby = await import('globby');
  const clientAssetsPath = path.join(path.dirname(runtimeSettings.resolvedClientPath), "assets");
  const indexFile = (await globby.globby(["index.*.js"], { cwd: clientAssetsPath }))?.[0];
  if (indexFile) {
    const indexPath = path.join(clientAssetsPath, indexFile);
...
const defaultConfig = {
  ...
  puppeteerClusterOptions: {
    ...
    maxConcurrency: Math.max(os__default.cpus().length - 1, 1),
...
async function discoverRouteDefinitions() {
  ...
  logger.debug(`Discovered \`${resolvedPages.length}\` page files from  \`${path.join(resolvedConfig.root, dir)}\`. Mapping to route definitions.`);
  ...
  await setup(() => {
    ...
    core.prefix("/api", () => {
      ...
      core.get("__launch", () => {
        ...
        const resolved = path.join(resolvedConfig.root, path$1);
        logger.info(`Launching file in editor: \`${path$1}\``);
        launch__default(resolved);
      });
      core.get("ws", (event) => ws.serve(event.req));
      core.get("reports", () => {
        const { worker } = useUnlighthouse();
...
function normaliseRoute(url) {
  ...
  const path$1 = `${ufo.withLeadingSlash($url.pathname)}${hash}${$url.search}`;
  let normalised = {
  ...
  for (const matcher in resolvedConfig.scanner.customSampling) {
    if (!new RegExp(matcher).test(path$1))
      continue;
    ...
    normalised = {
      ...
      definition: {
        ...definition,
        path: path$1,
        componentBaseName: path.basename(definition.component || "")
      }
    };
    break;
  }
...
const resolveReportableRoutes = async () => {
  ...
  const pathsChunkedToGroup = lodashEs.groupBy(
    validUrls.map((url) => normaliseRoute(url)),
    resolvedConfig.client.groupRoutesKey.replace("route.", "")
  );
  const pathsSampleChunkedToGroup = lodashEs.map(
    pathsChunkedToGroup,
    ...
    (group) => {
      ...
      return lodashEs.sampleSize(group, dynamicSampling);
    }
  );
  ...
  return function(path) {
    for (const v of [{ rules: include, result: true }, { rules: exclude, result: false }]) {
      ...
      if (stringRules.length > 0) {
        ...
        const routeRulesMatcher = radix3.toRouteMatcher(radix3.createRouter({ routes, strictTrailingSlash: false }));
...
const inspectHtmlTask = async (props) => {
  ...
  const logger = useLogger();
  let html;
  const htmlPayloadPath = path.join(routeReport.artifactPath, ReportArtifacts.html);
  if (resolvedConfig.cache && fs__default.existsSync(htmlPayloadPath)) {
    ...
    if (response.redirected) {
      ...
      if (ufo.withoutTrailingSlash(response.redirected) !== ufo.withoutTrailingSlash(runtimeSettings.siteUrl.href))
...
function normaliseLighthouseResult(route, result) {
  ...
  if (result.audits["screenshot-thumbnails"]?.details?.items) {
    for (const k in result.audits["screenshot-thumbnails"].details.items)
      result.audits["screenshot-thumbnails"].details.items[k].data = pathe.relative(runtimeSettings.generatedClientPath, path.join(route.artifactPath, ReportArtifacts.screenshotThumbnailsDir, `${k}.jpeg`));
  }
  return {
    // @ts-expect-error type override
    categories: lodashEs.map(result.categories, (c, k) => {
      return {
        key: k,
        id: k,
        ...lodashEs.pick(c, ["title", "score"])
      };
    }),
    ...lodashEs.pick(result, [
      "fetchTime",
      "audits.redirects",
      // core web vitals
    ...
    score: Math.round(lodashEs.sumBy(measuredCategories, "score") / measuredCategories.length * 100) / 100
  };
}
const runLighthouseTask = async (props) => {
  ...
  const { resolvedConfig, runtimeSettings } = useUnlighthouse();
  const { page, data: routeReport } = props;
  const reportJsonPath = path.join(routeReport.artifactPath, ReportArtifacts.reportJson);
  ...
  if (resolvedConfig.defaultQueryParams)
    clonedRouteReport.route.url = ufo.withQuery(clonedRouteReport.route.url, resolvedConfig.defaultQueryParams);
  const args = [
    `--cache=${JSON.stringify(resolvedConfig.cache)}`,
    `--routeReport=${JSON.stringify(lodashEs.pick(clonedRouteReport, ["route.url", "artifactPath"]))}`,
  ...
  if (report.audits?.["final-screenshot"]?.details?.data)
    await fs__default.writeFile(path.join(routeReport.artifactPath, ReportArtifacts.screenshot), base64ToBuffer(report.audits["final-screenshot"].details.data));
  if (report.fullPageScreenshot?.screenshot.data)
    await fs__default.writeFile(path.join(routeReport.artifactPath, ReportArtifacts.fullScreenScreenshot), base64ToBuffer(report.fullPageScreenshot.screenshot.data));
  const screenshotThumbnails = report.audits?.["screenshot-thumbnails"]?.details;
  await fs__default.mkdir(path.join(routeReport.artifactPath, ReportArtifacts.screenshotThumbnailsDir), { recursive: true });
  if (screenshotThumbnails?.items && screenshotThumbnails.type === "filmstrip") {
    for (const key in screenshotThumbnails.items) {
      ...
      await fs__default.writeFile(path.join(routeReport.artifactPath, ReportArtifacts.screenshotThumbnailsDir, `${key}.jpeg`), base64ToBuffer(thumbnail.data));
    }
  }
...
async function createUnlighthouseWorker(tasks) {
  ...
  const queueRoute = (route) => {
    ...
    if (resolvedConfig.scanner.dynamicSampling && resolvedConfig.scanner.dynamicSampling > 0) {
      const routeGroup = lodashEs.get(route, resolvedConfig.client.groupRoutesKey.replace("route.", ""));
      const routesInGroup = [...routeReports.values()].filter(
        (r) => lodashEs.get(r, resolvedConfig.client.groupRoutesKey) === routeGroup
      ).length;
    ...
    const runTaskIndex = (idx = 0) => {
      ...
      cluster.execute(routeReport, (arg) => {
        ...
        logger.success(`Completed \`${taskName}\` for \`${routeReport.route.path}\`. ${utils.colorize("gray", `(${reportData.join(" ")})`)}`);
        runTaskIndex(idx + 1);
      });
    };
    runTaskIndex();
  ...
  const queueRoutes = (routes) => {
    routes = lodashEs.uniqBy(routes, "path");
    const sortedRoutes = lodashEs.sortBy(routes, (r) => {
      if (resolvedConfig.discovery && r.definition)
        return lodashEs.get(r, resolvedConfig.client.groupRoutesKey.replace("route.", ""));
      return r.path;
    });
  ...
  const requeueReport = (report) => {
    ...
    Object.values(ReportArtifacts).forEach((artifact) => {
      fs__default$1.rmSync(path.join(report.artifactPath, artifact), { force: true, recursive: true });
    });
...
const resolveUserConfig = async (userConfig) => {
  ...
  config.chrome = defu.defu(config.chrome || {}, {
    ...
    downloadFallbackCacheDir: path.join(os.homedir(), ".unlighthouse")
  });
  if (config.auth) {
  ...
  if (config.client?.columns) {
    config.client.columns = lodashEs.pick(config.client.columns, ["overview", ...config.lighthouseOptions.onlyCategories]);
  }
  if (config.root && config.discovery && config.discovery.pagesDir === "pages") {
    const pagesDirExist = await fs.pathExists(path.join(config.root, config.discovery.pagesDir));
    if (!pagesDirExist) {
  ...
  config.puppeteerClusterOptions = defu.defu(config.puppeteerClusterOptions, {
    timeout: 12e4
  });
  config.puppeteerOptions = defu.defu(config.puppeteerOptions, {
    // set viewport
  ...
  if (foundChrome) {
    ...
    const instance2 = await puppeteer.launch(config.puppeteerOptions).catch((e) => {
  ...
  if (config.chrome.useDownloadFallback && !foundChrome) {
    const browserOptions = {
      ...
      buildId: config.chrome.downloadFallbackVersion || revisions_js.PUPPETEER_REVISIONS.chrome,
      browser: "chrome"
    };
  ...
  const instance = await puppeteer.launch(config.puppeteerOptions).catch((e) => {
    if (browsers.detectBrowserPlatform() === "linux" && e.toString().includes("error while loading shared libraries")) {
      const depsPath = path__default.join(
        path__default.dirname(config.puppeteerOptions.executablePath),
        "deb.deps"
      );
      if (fs$1.existsSync(depsPath)) {
        const data = fs$1.readFileSync(depsPath, "utf-8").trim().split("\n").map((d) => `"${d}"`).join(",");
...
${command}`);
  ...
  config.outputPath = path.resolve(config.root, config.outputPath);
  return config;
};
...
function box(message, title, options) {
  return `${utils.box([
    title,
    "",
    utils.colorize("white", foldLines(message, 0, 0, maxCharsPerLine()))
  ].join("\n"), Object.assign({
    borderColor: "white",
    borderStyle: "round",
    padding: 1,
...
async function createUnlighthouse(userConfig, provider) {
  ...
  const { __dirname } = mlly.createCommonJS((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('index.cjs', document.baseURI).href)));
  if (userConfig.root && !path.isAbsolute(userConfig.root))
    userConfig.root = path.join(process.cwd(), userConfig.root);
  ...
  const { configFile, config } = await c12.loadConfig({
    name: "unlighthouse",
  ...
  runtimeSettings.lighthouseProcessPath = await mlly.resolvePath(
    path.join(runtimeSettings.moduleWorkingDir, "process", "lighthouse.mjs")
  ).catch(() => "");
  if (!await fs__default.pathExists(runtimeSettings.lighthouseProcessPath)) {
    runtimeSettings.lighthouseProcessPath = await mlly.resolvePath(
      path.join(runtimeSettings.moduleWorkingDir, "process", "lighthouse.ts")
    );
  }
  ...
  ctx.setCiContext = async () => {
    ...
    try {
      await fs__default.mkdir(resolvedConfig.outputPath, { recursive: true });
    } catch (e) {
    ...
    ctx.runtimeSettings = {
      ...
      resolvedClientPath: await mlly.resolvePath(ClientPkg, { url: (typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('index.cjs', document.baseURI).href)) })
    };
    if (!resolvedConfig.cache && fs$1.existsSync(resolvedConfig.outputPath)) {
      ...
      fs__default.rmSync(outputPath, { recursive: true });
    }
  ...
  ctx.setServerContext = async ({ url, server, app }) => {
    ...
    if (!resolvedConfig.cache && fs$1.existsSync(ctx.runtimeSettings.outputPath)) {
      logger.debug(`\`cache\` is disabled, deleting cache folder: \`${ctx.runtimeSettings.outputPath}\``);
      try {
        fs__default.rmSync(ctx.runtimeSettings.outputPath, { recursive: true });
      } catch (e) {
  ...
  ctx.start = async () => {
    ...
    if (provider?.name !== "ci") {
      const label = (name) => utils.colorize("bold", utils.colorize("magenta", `\u25B8 ${name}:`));
      let mode = "";
      if (resolvedConfig.urls?.length)
        mode = "Manual";
      if (resolvedConfig.scanner.sitemap !== false)
      ...
      const title = [
        `\u26F5\u200D  ${utils.colorize("bold", utils.colorize("blueBright", AppName))} ${utils.colorize("dim", `${provider?.name} @ v${version}`)}`
      ];
      if (Number(latestTag.replace("v", "").replace(".", "")) > Number(version.replace(".", ""))) {
        title.push(...[
          "",
          `\u{1F389} New version ${latestTag} available! Use the latest:`,
          utils.colorize("gray", ` > ${utils.colorize("underline", `npx unlighthouse@^${latestTag} --site ${resolvedConfig.site}`)}`)
        ]);
      }
      title.push(...[
        "",
        `${label("Scanning")} ${resolvedConfig.site}`,
        `${label("Route Discovery")} ${mode} ${ctx.routes.length > 1 ? utils.colorize("dim", `${ctx.routes.length} initial URLs`) : ""}`,
        "",
        utils.colorize("dim", " \u{1F496} Like Unlighthouse? Support the development: https://github.com/sponsors/harlan-zw")
      ]);
      if (ctx.routeDefinitions?.length)
      ...
      process.stdout.write(successBox(
        // messages
        [
          ctx.runtimeSettings.clientUrl ? utils.colorize("whiteBright", `Report: ${ctx.runtimeSettings.clientUrl}`) : ""
        ].join("\n"),
        // title
        title.join("\n")
      ));
      if (fs$1.existsSync(path.join(ctx.runtimeSettings.generatedClientPath, "reports", "lighthouse.json")) && ctx.resolvedConfig.cache)
        logger.info(`Restoring reports from cache. ${utils.colorize("gray", "You can disable this behavior by passing --no-cache.")}`);
    }
    return ctx;
  };
  if (ctx.resolvedConfig.site)
...
function normaliseHost(host) {
  ...
  host = host.includes(".") ? host : ufo.withTrailingSlash(host);
  return new URL(host);
}
function createTaskReportFromRoute(route) {
  ...
  const reportPath = path.join(runtimeSettings.generatedClientPath, "reports", sanitiseUrlForFilePath(route.path));
...
async function createAxiosInstance(resolvedConfig) {
  dns__default.setServers([
    "8.8.8.8",
    // Google
    "1.1.1.1"
    // Cloudflare
  ]);
  const resolver = new dns__default.Resolver();
  resolver.setServers([
    "8.8.8.8",
    // Google
    "1.1.1.1"
  ...
  axiosOptions.httpAgent = new http__default.Agent({
    keepAlive: true,
    timeout: 3e4
  });
  ...
  unlighthouse._axios = axios__default.create(axiosOptions);
