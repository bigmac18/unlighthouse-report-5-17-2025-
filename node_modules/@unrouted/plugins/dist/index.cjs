...
const laravelNamedParams = core.defineUnroutedPlugin({
  meta: {
...
`;
    ...
    for (const route of methodStack[method]) {
      ...
      if (route.meta.resolve?.file) {
        const relativePath = pathe.relative(pathe.dirname(pluginConfig.outputPath), route.meta.resolve?.file).replace(/\.[^/.]+$/, "");
        imp = `Awaited<ReturnType<typeof import('${relativePath}').${route.meta.resolve?.fn}>>`;
      } else if (route.meta.resolve?.import) {
        const relativePath = pathe.relative(pathe.dirname(pluginConfig.outputPath), route.meta.resolve?.import.from).replace(/\.[^/.]+$/, "");
...
`;
  ...
  types += "}\n\n";
  types += "export {}\n";
  await fse__default.outputFile(pluginConfig.outputPath, types, { encoding: "utf-8" });
}
...
const generateTypes = core.defineUnroutedPlugin({
  meta: {
    name: "generateTypes"
  },
  defaults: {
...
const watchExports = core.defineUnroutedPlugin({
  meta: {
    name: "watchExports"
  },
  defaults: {
  ...
  async setup({ hooks }, options) {
    ...
    hooks.hook("setup:before", async () => {
      const imports = await unimport.scanDirExports(options.routesPath, {
...
function untypePayload(payload, options) {
  ...
  if (typeof payload === "number")
    return `${payload}|${typeof payload}`;
  return untyped.generateTypes(untyped.resolveSchema(payload), options);
}

const untypedPayloads = core.defineUnroutedPlugin({
  meta: {
...
exports.laravelNamedParams = laravelNamedParams;
exports.untypedPayloads = untypedPayloads;
exports.watchExports = watchExports;
