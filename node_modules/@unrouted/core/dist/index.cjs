...
function createLogger(key = "unrouted", debug = false) {
  const logger = consola__default.withScope(key);
  if (debug) {
    logger.level = 4;
  }
  return logger;
...
function redirect(route, location, code) {
  registerRoute("get", route, h3.eventHandler((e) => h3.sendRedirect(e, location, code)));
}
function permanentRedirect(route, location) {
...
function resolveStackRouteMeta() {
  const { groupStack } = useUnrouted();
  let meta = {};
...
function unroutedEventHandler(handle) {
  return h3.eventHandler(async (e) => {
    ...
    const timeTaken = (/* @__PURE__ */ new Date()).getTime() - now;
